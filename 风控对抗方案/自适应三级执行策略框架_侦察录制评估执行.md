...
> 以下工具经联网核查（2025-2026），按三阶段分类整理，注明可信来源。
...
| 库 | 安装 | 说明 | 来源 |
|---|---|---|---|
| `python-Wappalyzer` | `pip install python-Wappalyzer` | 调用 Wappalyzer 规则库（5000+ 技术特征），从 HTML + Headers 识别防作弊工具 | pypi.org |
| `wappalyzer` | `pip install wappalyzer` | CLI + Python 双用，规则库与官方浏览器插件同源 | pypi.org |
| `builtwith` | `pip install builtwith` | BuiltWith API 封装，可溯源 CDN / 防作弊历史技术栈 | pypi.org |
| `Techackz` | `git clone gotr00t0day/Techackz` | **四合一**：Wappalyzer + 自定义指纹 + BuiltWith + Nuclei 安全扫描 | GitHub |
| `curl_cffi` | `pip install curl_cffi` | Pass 1 HTTP 侦察，TLS/JA3 指纹伪装发出探测请求 | 本策略已用 |
| `beautifulsoup4` | `pip install beautifulsoup4` | Pass 2 HTML 源码扫描，提取 `<script src>` 域名特征 | 本策略已用 |

> `Techackz` 是侦察阶段覆盖最广的开源工具，比单独用 `python-Wappalyzer` 多出安全扫描维度；推荐在 Pass 2 扫描后以其结果补充验证。

#### 在线服务（人工辅助核验）

| 服务 | 用途 | 地址 |
|---|---|---|
| **urlscan.io** | 提交 URL → 自动截图 + 抓包 + 识别第三方脚本（含 DataDome / PerimeterX CDN 域名）| https://urlscan.io |
| **BuiltWith API** | 历史技术栈溯源，可见该域名过去换过哪些防作弊工具 | https://api.builtwith.com/v21/api.json?LOOKUP=domain |

#### 浏览器插件

| 插件 | 精准度 | 说明 |
|---|---|---|
| **Wappalyzer**（Chrome/Firefox）| ★★★★ | 识别前端框架、CDN、防作弊工具，与 python-Wappalyzer 同规则库 |
| **BuiltWith Extension** | ★★★ | 比 Wappalyzer 更详细的 tech stack 溯源 |
| **WhatRuns** | ★★★ | 轻量版，适合快速初筛 |
...
| 工具 | 类型 | HAR | DOM 操作 | DOM 变化 | Storage | WebSocket | Python 钩子 |
|---|---|---|---|---|---|---|---|
| **mitmproxy / mitmdump** | Python 代理 | ✅ | ❌ | ❌ | ❌ | ✅ | ✅ |
| **Playwright recordHar + codegen** | CLI | ✅ | ✅（生成脚本）| ❌ | ❌ | ⚠️ 有限 | ❌ |
| `page.add_init_script`（本策略）| JS 注入 | ❌ | ✅ | ✅ | ✅ | ❌ | — |
| **Chrome DevTools Recorder** | 浏览器内置 | ❌ | ✅（导出 PW 脚本）| ❌ | ❌ | ❌ | ❌ |
| **Reqable** | GUI 代理 | ✅ | ❌ | ❌ | ❌ | ✅ | ❌ |
| **Proxyman**（Mac）| GUI 代理 | ✅ | ❌ | ❌ | ❌ | ✅ | ❌ |

> **最优组合**：`mitmproxy`（HAR + API 钩子）+ `page.add_init_script`（DOM 事件 + MutationObserver + Storage）；两者互补，正是本策略 Phase 1 四轨录制方案的实现基础。

#### mitmproxy 录制最佳实践

```bash
# 启动为系统代理，同时写 HAR 文件 + 注入自定义 Python 脚本
mitmdump -w session.mitm --set hardump=session.har -s hook.py --listen-port 8080
```

```python
# hook.py：只记录 /api/ 路径下的接口
from mitmproxy import http
import json, pathlib

log = pathlib.Path("api_calls.jsonl").open("a")
...
def response(flow: http.HTTPFlow):
    if "/api/" in flow.request.path:
        log.write(json.dumps({
            ...
            "req":      flow.request.get_text()[:500],
            "status":   flow.response.status_code,
            "res":      flow.response.get_text()[:500],
...
| 工具 | 平台 | 亮点 |
|---|---|---|
| **Reqable** | Win/Mac/Linux | 免费，支持 gRPC + WebSocket，现代 UI，是 Charles 的最佳替代 |
| **Proxyman** | Mac only | SSL 解密配置最简单，支持 iOS/Android 设备 |
| **Charles Proxy** | Win/Mac | 最成熟，规则脚本强，但 UI 较老 |

---

### 评估阶段工具 / Tier 3 Stealth 工具选型

#### 2025 年 Stealth 工具代际分化

> 来源：castle.io《From Puppeteer stealth to Nodriver》，Jun 2025  
> **关键背景**：2022 年 11 月 Google 统一 headless/headful Chrome 代码库，JS API 层面的差异大幅消失。第一代 JS 补丁工具效果持续下降，第二代转向**协议层规避**。

##### 第一代（JS API 层打补丁）— 部分失效，仍可用于基础场景

| 库 | 安装 | 特点 | 局限 |
|---|---|---|---|
| `playwright-stealth` | `pip install playwright-stealth` | 本策略已用，`stealth_async(page)` 消除 webdriver 特征 | 无法突破 Cloudflare JS Challenge（zenrows 实测）|
| `playwright-extra` + stealth 插件 | `pip install playwright-extra` | 插件化 stealth，15+ 个补丁 | 已被高级防作弊系统反向工程，部分失效 |
| `selenium-stealth` | `pip install selenium-stealth` | Selenium 版 stealth | 效果弱于 playwright-stealth |

##### 第二代（协议层 / 浏览器级规避）— 当前主流

| 库 | 安装 | 核心机制 | 实测结果 | 来源 |
|---|---|---|---|---|
| **`patchright`** | `pip install patchright` | Playwright drop-in 替换；修补 HeadlessChrome UA 标志、navigator.webdriver、弹窗屏蔽；Python + Node.js | bot.sannysoft.com ✅；CF JS Challenge ❌ | zenrows, 2025 |
| **`undetected-chromedriver`** | `pip install undetected-chromedriver` | 动态 Chrome 配置文件，自动处理 CDP 协议检测信号 | 成功率 ~95%，优于 selenium-stealth | scrapingant, 2024 |
| **`SeleniumBase` UC 模式** | `pip install seleniumbase` | 封装 undetected-chromedriver，提供更高级 API 和内置重试 | 比裸 uc 更稳定 | scrapingant, 2024 |
| **`Camoufox`** | `pip install camoufox` | **基于 Firefox**（非 Chrome），浏览器级指纹修改，规避 Chrome 特有检测信号 | 对专检 Chrome 自动化的站点有奇效 | zenrows, 2025 |
| **`nodriver`** | `pip install nodriver` | **完全不使用 CDP**，通过 WebSocket 直连浏览器，消除 CDP 引入的所有检测信号 | 针对 Kasada 等 CDP 级检测最有效 | castle.io, Jun 2025 |
| **`Scrapling`** | `pip install scrapling` | 轻量 scraping 库，内置 stealth 能力，介于 requests 和 Playwright 之间 | 适合非重型任务 | zenrows, 2025 |

#### Tier 3 Stealth 工具选择树

```
目标站点使用 CDP 级检测（Kasada / 瑞数）?
  └─ 是 → nodriver（彻底绕开 CDP 协议层）

目标站点专门检测 Chrome 自动化特征（非通用检测）?
  └─ 是 → Camoufox（Firefox 内核，规避 Chrome 特有信号）

一般 Cloudflare / DataDome / PerimeterX / 数美 / 极验?
  ├─ 改动最小 → patchright（playwright drop-in 替换）
  └─ 稳定性优先 → undetected-chromedriver + SeleniumBase UC 模式

对应 Tier 关系：
  Tier 3 默认 → playwright-stealth + patchright
  Tier 3 强化 → nodriver（CDP 检测）/ Camoufox（Chrome 检测）
```
...
| 工具 | 对应 Tier | 说明 |
|---|---|---|
| `curl_cffi` | **Tier 1 / 2** | TLS/JA3/HTTP2 指纹伪装，纯协议层最强 |
| `playwright-stealth` + `patchright` | **Tier 3 基础** | drop-in 替换，改动最小 |
| `undetected-chromedriver` / `SeleniumBase UC` | **Tier 3 中级** | 动态配置文件，绕过 CDP 信号，成功率更高 |
| `Camoufox` | **Tier 3 中级（Firefox 路线）** | 专克 Chrome 检测工具；与 Playwright 生态不兼容 |
| `nodriver` | **Tier 3 高级** | 针对 Kasada 等 CDP 级检测，完全绕开 CDP 协议 |

#### 评估测试站点（验证 stealth 效果，可接入自动化金丝雀探测）

| 站点 | 检测内容 | 自动化方式 |
|---|---|---|
| **bot.sannysoft.com** | `navigator.webdriver`、Chrome runtime、plugins | Playwright 打开 → 截图 → 解析结果 |
| **browserleaks.com** | Canvas / WebGL / Font / AudioContext 指纹 | 同上 |
| **pixelscan.net** | IP 与浏览器指纹一致性综合评分 | 同上，验证代理 + 浏览器组合是否匹配 |
| **creepjs**（abrahamjuliot/creepjs）| Worker / iframe 跨域一致性，最全面 | 同上，发现隐蔽泄漏点 |
...
```diff
# ── 侦察阶段 ──
+ python-Wappalyzer>=0.3.1   # 技术栈识别（5000+ 规则）
+ builtwith>=1.3.5            # BuiltWith API 历史溯源（可选）
+ curl_cffi>=0.7.0            # Pass 1 HTTP 侦察 + Tier 1/2 执行
+ beautifulsoup4>=4.12.0      # Pass 2 HTML 扫描

# ── 录制阶段 ──
+ mitmproxy>=10.0.0           # HAR 录制 + Python 钩子拦截

# ── Tier 3 执行（按目标站点选其一）──
  playwright>=1.40.0          # 基础（主框架已有）
+ playwright-stealth>=1.0.0   # Tier 3 基础 stealth
+ patchright                  # Tier 3 基础（playwright drop-in 替换）
+ undetected-chromedriver     # Tier 3 中级（CDP 规避）
+ seleniumbase                # Tier 3 中级（UC 模式封装）
+ camoufox                    # Tier 3 中级（Firefox 路线，克 Chrome 检测）
+ nodriver                    # Tier 3 高级（无 CDP，克 Kasada / 瑞数）

# ── CAPTCHA Solver ──
+ httpx>=0.27.0               # CapSolver / 2captcha API 调用
```
